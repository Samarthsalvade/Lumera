...
const ResultsSkeleton = () => (
  <div className="max-w-5xl mx-auto">
    {/* Back link */}
    <Sk.Line className="w-36 h-4 mb-6" />

    {/* Main card */}
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
      <div className="md:flex">
        {/* Left — image area */}
        <div className="md:w-2/5 p-5 space-y-3 bg-gray-50">
          <Sk.Line className="w-32 h-3 mx-auto" />
          <Sk.Box className="w-full aspect-square rounded-xl" />
          <Sk.Line className="w-40 h-3 mx-auto" />
          <Sk.Line className="w-24 h-3 mx-auto" />
          <Sk.Box className="w-full aspect-square rounded-xl opacity-60" />
        </div>
        {/* Right — result info */}
        <div className="md:w-3/5 p-8 space-y-5">
          <Sk.Line className="w-48 h-8" />
          <Sk.Line className="w-36 h-3" />
          <div className="flex items-center gap-3">
            <Sk.Line className="w-32 h-10" />
            <Sk.Box className="w-20 h-7 rounded-full" />
          </div>
          <div className="space-y-2">
            <div className="flex justify-between">
              <Sk.Line className="w-24" />
              <Sk.Line className="w-12" />
            </div>
            <Sk.Box className="h-3 w-full rounded-full" />
          </div>
          {/* Concern chips */}
          <div className="flex flex-wrap gap-2">
            {[1, 2, 3].map(i => <Sk.Box key={i} className="h-7 w-28 rounded-full" />)}
          </div>
          {/* Recommendation lines */}
          <div className="space-y-2">
            <Sk.Line className="w-36 h-3" />
            {[1, 2, 3].map(i => <Sk.Box key={i} className="h-12 w-full" />)}
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
      <div className="flex border-b border-gray-100">
        {[1, 2, 3].map(i => <Sk.Box key={i} className="flex-1 h-14 rounded-none" />)}
      </div>
      <div className="p-6 space-y-4">
        {[1, 2, 3].map(i => (
          <div key={i} className="flex gap-4 p-4 bg-gray-50 rounded-xl">
            <Sk.Box className="w-32 h-32 flex-shrink-0" />
            <div className="flex-1 space-y-3">
              <Sk.Line className="w-40 h-5" />
              <Sk.Box className="h-2 w-full rounded-full" />
              <Sk.Line className="w-full h-3" />
              <Sk.Line className="w-3/4 h-3" />
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="flex gap-3 justify-center">
      {[1, 2, 3].map(i => <Sk.Box key={i} className="w-36 h-12" />)}
    </div>
  </div>
);

// ─── All original interfaces and sub-components (unchanged) ──────────────────
...
const ProductImageTile = ({ imageUrl, productName, brand }: { imageUrl: string; productName: string; brand: string }) => {
  ...
  if (isRealImage) return (
    <div className="w-full h-36 rounded-xl overflow-hidden bg-gray-50 flex items-center justify-center">
      <img src={imageUrl} alt={productName} onError={() => setImgFailed(true)} className="w-full h-full object-contain p-2"/>
    </div>
  );
  return (
...
const CONCERN_META: Record<string, { label: string; color: string; icon: JSX.Element }> = {
  ...
  blackheads:            { label: 'Blackheads',            color: 'bg-gray-50 border-gray-300 text-gray-800',     icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><circle cx="12" cy="12" r="1"/><circle cx="6" cy="8" r="1"/><circle cx="18" cy="8" r="1"/><circle cx="6" cy="16" r="1"/><circle cx="18" cy="16" r="1"/></svg> },
...
const HorizontalSlider = ({ children, itemWidth = 260 }: { children: React.ReactNode[]; itemWidth?: number }) => {
  ...
  return (
    <div className="relative">
      {canLeft && (
        <button onClick={() => scroll('left')} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3 z-10 w-8 h-8 bg-white border border-gray-200 rounded-full shadow flex items-center justify-center hover:bg-gray-50 transition">
      ...
      <div ref={trackRef} className="flex gap-4 overflow-x-auto pb-2 scroll-smooth" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        {children}
      </div>
      {canRight && (
        <button onClick={() => scroll('right')} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-3 z-10 w-8 h-8 bg-white border border-gray-200 rounded-full shadow flex items-center justify-center hover:bg-gray-50 transition">
...
const Results = () => {
  ...
  const loadProducts = async () => {
    ...
    try {
      const activeConcerns = concerns.filter(c => c.confidence > 0.15).map(c => ({ concern_type: c.concern_type, severity: c.severity }));
      const res = await api.post('/products/recommend', { skin_type: analysis.skin_type, concerns: activeConcerns, count: 5 });
      setDynProducts(res.data.products || []);
      setProductsLoaded(true);
    } catch {
      setDynProducts([]); setProductsLoaded(true);
  ...
  const generateRoutine = async () => {
    ...
    try {
      ...
      setRoutineError(err.response?.data?.error || 'Generation failed.');
  ...
  if (loading) return <PageShell><ResultsSkeleton /></PageShell>;

  if (!analysis) return (
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center">
      <div className="text-center">
        <h2 className="text-2xl font-bold mb-4">Analysis not found</h2>
        <Link to="/dashboard" className="text-purple-600 hover:underline text-base">Back to Dashboard</Link>
      </div>
    </div>
  );
                  ...
                  {productsLoading ? (
                    ...
                    <div className="px-2">
                      <HorizontalSlider itemWidth={272}>
                        {dynProducts.map((p, i) => (
                          <div key={i} className="flex-shrink-0 w-64 border border-gray-100 rounded-xl overflow-hidden hover:border-purple-200 hover:shadow-md transition flex flex-col">
                            <div className="p-3 pb-0"><ProductImageTile imageUrl={p.amazon_image_url} productName={p.product_name} brand={p.brand}/></div>
                            <div className="p-4 flex flex-col flex-1">
                              <div className="flex justify-between items-start mb-1">
                                ...
                                <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize flex-shrink-0 ml-2 ${PRICE_BADGE[p.price_range] ?? 'bg-gray-100 text-gray-600'}`}>{p.price_range}</span>
                              </div>
                      ...
                      <div className="flex justify-center gap-1.5 mt-4">
                        {dynProducts.map((_, i) => <div key={i} className={`w-1.5 h-1.5 rounded-full ${i === 0 ? 'bg-purple-600' : 'bg-gray-200'}`}/>)}
                      </div>
                    </div>
                  )}
