  ...
  <div className="max-w-4xl mx-auto">
    ...
    <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div className="space-y-2">
        <Sk.Line className="w-24 h-4" />
        <Sk.Line className="w-40 h-8" />
        <Sk.Line className="w-64 h-4" />
      </div>
      <Sk.Box className="w-44 h-12 self-start" />
    </div>

    {/* Morning group label */}
    <div className="flex items-center gap-2 mb-4">
      <Sk.Circle className="w-5 h-5" />
      <Sk.Line className="w-36 h-5" />
      <Sk.Box className="w-6 h-5 rounded-full" />
    </div>

    {/* Morning routine cards */}
    {[1, 2].map(i => (
      <Sk.Card key={`m${i}`} className="mb-3 overflow-hidden">
        {/* Card header row */}
        <div className="px-6 py-5 flex items-center justify-between">
          <div className="flex-1 space-y-2">
            <div className="flex items-center gap-2">
              <Sk.Line className="w-48 h-5" />
              {i === 1 && <Sk.Box className="w-14 h-5 rounded-full" />}
            </div>
            <Sk.Line className="w-64 h-3" />
            <Sk.Line className="w-16 h-3" />
          </div>
          <Sk.Box className="w-5 h-5 ml-4 flex-shrink-0" />
        </div>
        {/* First card shows expanded steps */}
        {i === 1 && (
          <div className="border-t border-gray-100 px-6 py-5 space-y-3">
            {[1, 2, 3, 4].map(j => (
              <div key={j} className="flex gap-4 bg-purple-50/40 rounded-xl p-4">
                <Sk.Circle className="w-9 h-9 flex-shrink-0" />
                <div className="flex-1 space-y-2">
                  <Sk.Line className="w-36 h-4" />
                  <Sk.Line className="w-full h-3" />
                  <Sk.Line className="w-3/4 h-3" />
                  <div className="flex gap-3">
                    <Sk.Line className="w-10 h-3" />
    ...
    <Sk.Card className="mb-3 overflow-hidden">
      <div className="px-6 py-5 flex items-center justify-between">
        <div className="flex-1 space-y-2">
          <Sk.Line className="w-52 h-5" />
          <Sk.Line className="w-60 h-3" />
          <Sk.Line className="w-16 h-3" />
        </div>
        <Sk.Box className="w-5 h-5 ml-4 flex-shrink-0" />
      </div>
