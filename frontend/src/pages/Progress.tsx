...
function isoDate(d: string) { return d.slice(0, 10); }
      ...
      <div className="divide-y divide-gray-100">
        {[1,2,3,4,5].map(i => (
          <div key={i} className="flex items-center gap-6 px-6 py-4">
            ...
            <Sk.Box className="w-32 h-2 rounded-full" />
          ...
          <div className="max-w-6xl mx-auto">
            ...
            {analyses.length === 0 ? (
              <div className="bg-white rounded-2xl shadow-sm p-14 text-center">
                ...
                <Link to="/upload" className="inline-block bg-purple-600 text-white px-6 py-3 rounded-xl hover:bg-purple-700 transition font-medium text-base">Create First Analysis</Link>
              </div>
            ) : (
              <>
                {/* Stats */}
                ...
                <div className="grid lg:grid-cols-3 gap-6 mb-7">
                  <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm p-6">
                    ...
                    <div className="grid grid-cols-7 mb-2">
                      {DAY_NAMES.map(d => <div key={d} className="text-center text-sm font-medium text-gray-400 py-1">{d}</div>)}
                    </div>
                    <div className="grid grid-cols-7 gap-1">
                      ...
                      {Array.from({ length: daysInMonth }).map((_, i) => {
                        ...
                        return (
                          <button key={day}
                            ...
                            <span>{day}</span>
                            {scans.length > 0 && <span className="w-1.5 h-1.5 rounded-full mt-0.5" style={{ background: dotColor ?? '#a855f7' }} />}
                            {scans.length > 1 && <span className="absolute top-1 right-1 text-xs font-bold text-purple-600">{scans.length}</span>}
                          </button>
                        );
                      })}
                    </div>
                    <div className="mt-4 pt-4 border-t border-gray-100 flex items-center gap-4 flex-wrap">
                      ...
                      {Object.entries(SKIN_COLORS).slice(0, 3).map(([type, col]) => (
                        <span key={type} className="flex items-center gap-1.5 text-sm text-gray-600">
                          <span className="w-2.5 h-2.5 rounded-full" style={{ background: col.bar }} />{type}
                        </span>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-sm p-6 flex flex-col" style={{ minHeight: '420px' }}>
                    {!selectedDay ? (
                      ...
                      <div className="flex flex-col h-full">
                            ...
                            return (
                              <Link key={scan.id} to={`/results/${scan.id}`}
                                ...
                                <div className="flex items-center justify-between mb-1">
                                  <span className={`text-sm font-semibold px-2 py-0.5 rounded-full ${col.bg} ${col.text}`}>{scan.skin_type}</span>
                        ...
                        {selectedScans.length > 3 && <p className="text-xs text-gray-400 text-center mt-2 flex-shrink-0">Scroll to see all {selectedScans.length} scans</p>}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm p-6 mb-7">
                  ...
                  <div className="space-y-4">
                    {Object.entries(analyses.reduce((acc, a) => { acc[a.skin_type] = (acc[a.skin_type] || 0) + 1; return acc; }, {} as Record<string, number>))
                ...
                {/* History table */}
                <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                  <div className="p-6 border-b border-gray-100"><h2 className="text-xl font-bold">Scan History</h2></div>
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead className="bg-gray-50">
                        <tr>{['Date', 'Skin Type', 'Confidence', 'Action'].map(h => <th key={h} className="px-6 py-3.5 text-left text-sm font-semibold text-gray-500 uppercase tracking-wider">{h}</th>)}</tr>
                      </thead>
